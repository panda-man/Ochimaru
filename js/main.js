...
window.onload = function() {
    ...
    game.fps = 24;
    game.preload(['./image/chara1.gif','./image/icon0.gif','./image/bg.png','./image/chara-chi-maru.png']);
    //プリロードする画像を相対パスで指定
    ...
    // プリロード終了後に呼ばれる関数を指定する
        ...
        bear = new Sprite(50, 50);  // 50x50サイズの Sprite オブジェクトを生成
        bear.x = 0;                 // Sprite の左上のx座標を指定
        bear.y = 200;               // Sprite の左上のy座標を指定
        bear.width = 50;
        bear.height = 50;

        bear.image = game.assets['./image/chara-chi-maru.png']; // 画像を指定
        bear.frame = 0;
        ...
        background.image = game.assets['./image/bg.png'] // bg.png を指定
...
function addBanana(pos){
    ...
    banana.x = rand(300);               // 0 から 319 のあいだの乱数
    banana.y = 0;
    banana.image = game.assets['./image/icon0.gif'];
